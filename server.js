...
console.log("JWT_SECRET loaded by server:", SECRET);
...
async function ensureAdminUser() {
  try {
    const adminEmail = "ezecharles2468@gmail.com";
...
app.post("/api/properties", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (invalidImages.length > 0) {
      return res
        .status(400)
        .json({ error: "All images must be valid base64 data URLs" });
    }
  ...
  try {
    ...
    if (req.user.role !== "admin") {
      ...
      if (propertyResult.rows[0].user_id !== req.user.id) {
        return res
          .status(403)
          .json({ error: "Only the owner or admin can edit this property" });
      }
    }
...
app.post("/api/favorites/:propertyId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (err.code === "23505") {
      return res
        .status(400)
        .json({ error: "Property is already in favorites" });
    }
